<template>
  <div class="container mx-auto flex flex-col items-center bg-gray-100 p-4">
    <div class="fixed w-100 h-100 opacity-80 bg-purple-800 inset-0 z-50 flex items-center justify-center" v-if="!coinsArray">
      ...
      <section>
        <div class="flex">
          <div class="max-w-xs">
            ...
            <div class="flex bg-white shadow-md p-1 rounded-md shadow-md flex-wrap" v-if="hint.length">
              <template
                v-for="(h, idx) in hint"
                :key="h">
                <span 
            ...
            <div class="text-sm text-red-600" v-if="error">Такой тикер уже добавлен</div>
          </div>
        </div>
        <button
          type="button"
  ...
  methods: {  
    ...
    async fetchCoinsData(){
      const f = await fetch(
        ...
        this.coinsArray = Object.keys(data.Data)
    },

    typeText(){
      this.error = false
      this.hint = this.coinsArray.filter(el => 
        el.indexOf(this.ticker.toUpperCase()) !== -1
      )
    ...
    choiceHint(choicedHint){
      this.error = false
      this.ticker = choicedHint
      this.add()      
    }
